...
This directory freezes the lead certificate outputs used by the current paper
package. The original release tag is `v0.1-static-patch-diagnostics`; the
current artifact refresh corresponds to the post-v0.1 expert-cleanup and
...
| `approximate_cutoff_embeddings.json` | `PYTHONPATH=. python3 -m qgtoy static-patch-embedding-channels --max-cutoff 5`; includes trace-filled, harmonic, heat-kernel, and Berezin-inspired finite refinement audits |
...
and the generation baseline commit.
