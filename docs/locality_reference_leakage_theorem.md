...
Status: **PROVED as a bounded commuting-cell theorem and as a microcausal
corollary for bounded observables in a local net.** The elementary projected
commutator identity is standard compression/Toeplitz machinery. The candidate
paper result is the state-weighted three-cell theorem, its global `SO(3)`
orientation-risk corollary, and the constant-factor distributed realization.
Priority remains conditional on specialist review; no unbounded-current
...
We allow uniform collective-action errors
...
`A_a in A(O_a)` be bounded self-adjoint observables with
...
For any density operator `rho=P rho P`, define the omitted-mode weights
...
These are state-dependent squared off-code amplitudes. They are not yet
transition probabilities because the `A_a` have not been normalized as Kraus
operators or embedded in a time evolution.

The number `Lambda` in (3.1) must be a proved upper bound on the off-code
blocks; it is not a freely tunable parameter. The coarse choice `Lambda=M` is
always valid.
...
The theorem is class-uniform over the ambient Hilbert space, the finite code,
the code state, and all bounded microscopic realizations satisfying (3.1).
Tensoring the entire realization with an identity action on an arbitrary
rotation-trivial spectator leaves every norm and weight unchanged.
...
Suppose instead that every pair has locality defect at most `delta` and
...
```text
sum_a p_a(rho)/M^2>=g^4 [r^(-1)-8]_+/64.              (4.5)
```
...
For completeness, the risk input is not a local Cramer-Rao estimate. After
covariantizing an arbitrary POVM, write
`v_j=sqrt[Tr(P_j rho)]`. The spin-1 fusion matrix `F` for the character score
has `F_00=0`, `F_jj=1` for `j>=1`, and nearest-neighbor entries one. The exact
Dirichlet identity and discrete Hardy estimate are

```text
3-v^T F v=sum_j(v_j-v_(j+1))^2+2v_0^2,
sum_j(v_j-v_(j+1))^2>=1/[4 Tr(rho J^2)+2].             (4.6)
```

Since `R_ref=(3-score)/4`, (4.1) follows for arbitrary mixed states,
measurements, and rotation-trivial multiplicities. A complete proof is in
`docs/global_so3_reference_risk.md`; the paper must reproduce this lemma or
cite an independently published result with the same task and constant.
...
## 5. Distributed Near-Sharp Model
...
For distinct sites `i!=k`, permutation symmetry and the spin-one-half product
law give the useful identity
...
```text
Lambda^2=max_a ||Q A_a P_N||^2=N^2/[2(N-1)],
...
The theorem's Lambda-bound and its actual value are
...
Thus the main state-weighted theorem has the correct `Theta(N)` scaling in a
strictly local distributed model, within a constant factor eight. This does
not prove optimality of the factor four in (3.6).
...
a positive buffer. They do not claim local preparation, a continuum net, or a
finite-time lifetime.
...
The paper-level novelty candidate is specifically the conjunction of
pairwise spacelike cell replication, state-weighted omitted-mode excitation,
the global `SO(3)` risk/Casimir theorem, a pairwise-sharp two-cell model, and a
constant-factor three-cell realization.
