...
`literature_matrix.md`. This audit tests conceptual distinctions and external
claim discipline. It does not revalidate theorem proofs, artifact hashes, or
the primary-source searches recorded in the literature verification log.
...
1. Replaced “directional-record information” with the narrower
   “operational rotational-asymmetry resource” in the packet, review question,
   email, and diagram.
2. Replaced the obsolete missing-origin/partial-adjoint status with the
   completed response interval, which contains zero after all frozen origin,
   bulk, and wall terms are included.
3. Narrowed the Skyrmion claim from making the realization problem nonempty to
   making only the supported-profile subproblem nonempty.
4. Recorded the predeclared **INCONCLUSIVE STOP**, the roughly `9.50`-fold
   primal certificate gap, and the requirement for a materially new proof
...
- The compiled PDF and artifact manifest were regenerated after the source
  corrections above. Their current hashes are recorded by the packet audit;
  the separate requirement audit checks that the sendable artifacts and proof
  render describe the same source state.
- “Observer register” remains a prospective architectural term, not a proved
  identification with Harlow's observer subsystem. The packet marks that
  bridge open, but a reader who skips the boundary boxes could still overread
  the title.
