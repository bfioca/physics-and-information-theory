# Locality-Reference-Leakage Novelty Audit
...
covariant-code theorem after translating `p_a`, `Lambda`, and `alpha`.
...
> component of the same rigid finite `SO(3)` collective mode, then the omitted
> local modes carry a quantitatively bounded state-dependent excitation
> weight. For any code state, that weight is bounded below by its mean Casimir
> and hence by its achieved global orientation risk.
...
sum_a Tr[rho P A_a Q A_a P]
 ...
 >=alpha^4 Tr(rho J^2)/(4M^2),                         (3.1)
```

under exact pairwise spacelike commutation, exact compressed gain `alpha`, and
a certified `0<Lambda=max_a||Q A_aP||<=M`. Combining it with the existing
Haar-risk theorem gives

```text
sum_a Tr[rho P A_a Q A_a P]
 >=alpha^4 [R_ref^(-1)-8]_+/(64Lambda^2).             (3.2)
```

The consistency bound `sum_a p_a<=3Lambda^2` also yields

```text
Lambda^4>=alpha^4 [R_ref^(-1)-8]_+/192.               (3.3)
```

No checked primary source in this audit states this particular conjunction of
spacelike cell replication, state-weighted off-band excitation, full-frame
global `SO(3)` risk, and a distributed constant-factor realization. A
three-block ferromagnetic family has actual-to-bound ratio approaching eight;
a separate two-block family makes the pairwise norm coefficient two
asymptotically optimal. That distinction is narrow enough to defend and
concrete enough for a specialist to falsify.

## 4. Required Novelty Gates

Before submission, all of the following must hold:

1. An AQFT/QEC specialist confirms that (3.1)-(3.2) are not an immediate named
   corollary already standard in approximate cleaning or covariant-code theory.
...
6. A source-level comparison either proves that (3.1)-(3.3) are not immediate
   corollaries of Faist et al., Liu-Zhou, or approximate cleaning, or kills the
   novelty claim.
