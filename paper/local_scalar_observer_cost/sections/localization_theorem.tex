\section{The sharp localization--energy theorem}
\label{sec:bound}

Let $P_L$ be multiplication by $(0,L)$ on $L^2(\R_+)$.  We first record the
zero-temperature inverse kernels and then sum the complete KMS kernel.
...
We can now state the sharp cost, rather than only an upper bound.  The
supremum below is unchanged if smooth compact data are replaced by their
finite-energy closure.

\begin{theorem}[Sharp compact-support dephasing cost]
\label{thm:main}
Let
\begin{equation}
 C_{\rm opt}(y)
...
an overall sign.  The following fully explicit bounds hold:
\begin{align}
...
\end{equation}
then the sharper Schur bound $C_{\rm opt}(y)\leq2yM(y/2)$ holds.  The maximum
is attained at
\begin{equation}
...
\end{equation}
Finally, the exact support asymptotics obey the global remainder bounds
\begin{align}
...
\begin{proof}
For momentum data in the s-wave, \cref{lem:exact-kms-kernel} gives
\begin{equation}
...
\cref{eq:sharp-cost} and its lower bounds.

For the first explicit upper bound, use $\coth z\leq1+z^{-1}$,
\cref{lem:compressed-inverses,lem:angular-domination}, and the factor $1/2$ in
the energy to obtain
\begin{equation}
 \Gamma\leq\EK R F(y).
 \label{eq:legacy-explicit-bound}
\end{equation}
Positivity of $k_\tau$ and Schur's test give the $M(\tau)$ bound.  If
$f(s)=\log\sinh(\tau s)$, differentiation of the row integral gives
$[f(1+u)+f(1-u)-2f(u)]/\pi$.  The identity
\begin{equation}
 \sinh[\tau(1+u)]\sinh[\tau(1-u)]
 =\sinh^2\tau-\sinh^2(\tau u)
\end{equation}
then gives the unique stationary point \cref{eq:thermal-row-maximizer}.

For small support, write $k_\tau=k_0+d_\tau$.  Euler's product implies, for
$A\geq B\geq0$,
\begin{equation}
 0\leq\log\frac{\sinh A/A}{\sinh B/B}
 \leq\frac{A^2-B^2}{6}.
\end{equation}
Here $A=\tau(u+v)$ and $B=\tau\abs{u-v}$, so Schur's test gives
$0\leq\Lambda(\tau)-\Lambda(0)\leq\tau^2/(3\pi)$.  With $\tau=y/2$ this is
\cref{eq:small-support-exact}.

For large support, write
\begin{equation}
 k_\tau(u,v)=\frac{2\tau}{\pi}\min(u,v)+r_\tau(u,v).
\end{equation}
The remainder is nonnegative in operator order by the resolvent expansion.
Using $\int_0^\infty-\log(1-e^{-s})\dd s=\pi^2/6$ in its explicit row
integral gives $\norm{r_\tau}\leq\pi/(4\tau)$.  Since the norm of the
$\min(u,v)$ operator is $4/\pi^2$, multiplication by $2y=4\tau$ proves
\cref{eq:large-support-exact}.
\end{proof}
...
\begin{corollary}[Source-radius and duration bound]
