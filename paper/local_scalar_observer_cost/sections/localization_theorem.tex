...
\end{equation}
The remainder has the explicit expansion
\begin{equation}
 r_\tau(u,v)=\frac1\pi\sum_{n\geq1}\frac1n
 \left[e^{-2n\tau\abs{u-v}}-e^{-2n\tau(u+v)}\right].
 \label{eq:large-support-resolvent-series}
\end{equation}
Each bracket is a positive multiple of the Dirichlet half-line resolvent
kernel of $-\partial_u^2+(2n\tau)^2$, so $r_\tau\geq0$ in operator order.
For $a>0$,
\begin{equation}
 \int_0^\infty
 \left[e^{-a\abs{u-v}}-e^{-a(u+v)}\right]\dd v
 =\frac{2(1-e^{-au})}{a}\leq\frac2a.
\end{equation}
Monotone summation and Schur's test therefore give
$\norm{r_\tau}\leq(\pi\tau)^{-1}\sum_{n\geq1}n^{-2}
=\pi/(6\tau)$.  Since the norm of the $\min(u,v)$ operator is $4/\pi^2$,
multiplication by $2L$ proves \cref{eq:general-large-support}.
\end{proof}
