...
\subsection{Observer entropy and the simple random code}

Harlow, Usatyuk, and Zhao formulate quantum mechanics in a closed universe
using a quantum-to-classical observer channel and test that rule in a simple
Haar-random code~\cite{HarlowUsatyukZhao2025}.  Their second-moment expression
contains the observer-record factor
$e^{-S_2(\omega_{\rm Ob'})}=\Tr\omega_{\rm Ob'}^2$.  It is this factor, rather
than the binary diamond distance from complete dephasing, that makes contact
with the present field model.

The identification in \cref{eq:physical-purity-identification} is exact
because the controlled-displacement record is pure before either subsystem is
traced out.  The insertion in \cref{prop:harlow-floor} then specializes the
matter states so that the two additional overlaps in their formula vanish.
Thus our lower floor concerns a Haar-averaged squared inner-product
fluctuation for one orthogonal CRT-real pair.  We do not infer a deterministic
floor for every code realization, a uniform statement over the matter Hilbert
space, or a gravitational derivation of the quantum-to-classical channel.

The entropy also has a different role from a thermodynamic or area entropy.
$S_2(\rho_P)$ measures the mixedness of the pointer after its conditional
field records are discarded; its maximum is the classical collision entropy
$H_2(w)$.  The branchwise gravity corollary bounds this operational record
entropy by a local final-slice backreaction budget.  It does not equate the two
quantities or assume a Bekenstein--Hawking formula for the pointer.
