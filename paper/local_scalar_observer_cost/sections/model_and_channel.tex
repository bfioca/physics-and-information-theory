...
Zhao~\cite{HarlowUsatyukZhao2025} and its Hartle--Hawking
extension~\cite{Harlow2026Observers}.  Those works use the channel inside a
gravitational rule whose accuracy is controlled by observer entropy.  Here the
same binary channel target is realized by a prescribed scalar coupling.  The
error $\eobs$ measures this channel approximation only; it is not their closed-universe
encoding error and is not identified with $e^{-S_{\rm Ob}}$.
