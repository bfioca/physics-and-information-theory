...
constrained purity modifies the same random-code second moment.  Its purity
then supplies the entropy parameter in the simple random-code calculation of
Harlow, Usatyuk, and Zhao~\cite{HarlowUsatyukZhao2025}.
...
let $\ket\phi$, $\ket\psi$ be orthogonal CRT-real matter states.  Under the
controlled nonideal-record substitution in
\cref{eq:physical-record-state}, their exact second-moment formula gives
\begin{equation}
...
The proposition closes a specific algebraic bridge: after the ideal clone is
replaced in the simple code model, the entropy suppressing the fluctuation is
generated by a localized field record with an exact energy--support bound.
Its scope is narrower than a gravitational observer theorem.  The system
called ${\rm Ob'}$ in \cref{eq:physical-record-state} is the Stinespring
environment of the nonideal partial-dephasing channel, and
