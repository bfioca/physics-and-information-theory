...
the finite-pointer purity bound and its controlled substitution into a simple
observer code.
...
\begin{enumerate}[leftmargin=*,itemsep=2pt]
 ...
 \item the nonideal-record substitution
...
The code substitution supplies a concrete bridge that the earlier binary
diamond-error comparison did not.  The conditional field states are generally
nonorthogonal, so their record is not the exact pointer-basis clone used in the
ideal motivation.  Instead, their purification is the Stinespring record of a
partial-dephasing channel, with record-system purity exactly $\Tr\rho_P^2$.
Putting that record in the same bipartite slot is a controlled modification of
the simple code model.  For the specified orthogonal CRT-real pair, its purity
is precisely the observer factor left in the Haar second moment of
