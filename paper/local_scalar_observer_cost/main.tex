...
\begin{abstract}
The observer rule of Harlow, Usatyuk, and Zhao is motivated by an ideal
pointer-basis record and controlled by a second R\'enyi entropy, but does not
price a nonideal localized replacement.  We study one such replacement in an
exactly solvable local field model.  A finite pointer controls compact sources
for a free scalar,
...
When the Stinespring record of this partial-dephasing channel is substituted
for the exact pointer-basis clone in the simple Harlow--Usatyuk--Zhao random
code, the Haar-averaged squared inner-product fluctuation for an orthogonal
CRT-real matter pair is bounded below by the displayed physical purity, up to
the exact finite-code factor $D/(D+2)$.  This is a controlled nonideal-record
substitution, not an identification of the two records.  Finally, if every
conditional spherical $q_i=0$ branch obeys a
...
\input{sections/finite_pointer_entropy}
